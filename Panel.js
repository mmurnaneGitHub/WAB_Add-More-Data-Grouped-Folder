  ...
  function (
    ...
    return declare([BaseWidgetPanel, FoldableDijit], {
      ...
      onOpen: function () {
        ...
        if (this.config.label == 'Add More Data') { //MJM - adjust group folder behavior
          if (this.panelOpened > 0) {
            this._mjmGroupFolder();
            this.panelOpened++ //MJM - increment counter - just need to know if first time (0) - avoid all the other widget panels from being counted by using label check (Add More Data)
          } else {
            this.panelOpened++ //MJM - increment counter - just need to know if first time (0)
          };
        };  //End MJM
      },
